...
if pagina == 'Analyse geluid':
    ...
    # Copy Boeing 737-800 dataset
    df = data_1jaar_boeing737800.copy()

    # Clean the tags column
    df['tags_cleaned'] = df['tags'].fillna('').astype(str).str.split(',')
    df = df.explode('tags_cleaned')
    df['tags_cleaned'] = df['tags_cleaned'].str.strip()
    df = df[df['tags_cleaned'] != '']
    df = df.reset_index(drop=True)

    # Filter for those within 50km ground distance
    df = df[df['distance ground'] < 50]

    # Get top 10 types and operators
    top_10_types = df['type'].value_counts().nlargest(10).index.tolist()
    top_10_operators = df['operator'].value_counts().nlargest(10).index.tolist()

    # Axis and grouping selections
    numeric_colsx = ['distance', 'altitude', 'windspeed', 'winddirection', 'duration', 'TG_C']
    numeric_colsy = ['lasmax_dB', 'SEL_dB']

    st.title("✈️ Boeing 737-800: Scatterplot van Geluid en Vluchtgegevens")
    grouping_col = st.selectbox("🎨 Kleur op basis van:", ['landen_stijgen', 'tags_cleaned', 'operator'])
    x_axis = st.selectbox("Selecteer X-as", numeric_colsx, index=0)
    y_axis = st.selectbox("Selecteer Y-as", numeric_colsy, index=1)

    # Filter to top 10 types or operators
    if grouping_col == 'type':
        df = df[df['type'].isin(top_10_types)]
    elif grouping_col == 'operator':
        df = df[df['operator'].isin(top_10_operators)]

    # Drop NA for selected columns
    plot_data = df[[x_axis, y_axis, grouping_col]].dropna()
    ...
    fig = px.scatter(
        ...
        y=y_axis,
        color=grouping_col,
        title=f"{y_axis} vs {x_axis} per {grouping_col}",
        opacity=0.7,
        height=600
    )

    fig.update_layout(legend_title_text=grouping_col)
